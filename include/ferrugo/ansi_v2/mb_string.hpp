...
struct mb_string_list : public std::vector<mb_string>
{
    using base_t = std::vector<mb_string>;

    using base_t::base_t;

    mb_string_list(const std::string& text)
    {
        mb_string_list result;

        std::stringstream ss{ text };

        for (std::string line; std::getline(ss, line);)
        {
            this->push_back(mb_string(line));
        }
    }

    friend std::ostream& operator<<(std::ostream& os, const mb_string_list& item)
    {
